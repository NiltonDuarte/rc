...
class rcGraph:
	...
	def dinamicGCCMetrics(self, totalSteps = 26, intialTime = 126230461, timeInterval = 31536000):
		GCCsizes = []
		step = 0
		distanceList = []
		degreeHistList = []
	...
	def dinamicGCCMetrics1(self, totalSteps = 26, intialTime = 126230461, timeInterval = 31536000):
		GCCsizesList = []
		step = 0
		distanceList = []
		degreeHistList = []
		...
		while (step < totalSteps):
			...
			frequencyDegreeHist = self.degreeDistribution(degreeHist[0])
			print "calculated frequencyDegreeHist"
			GCCsizesList.append(gccVertices)
			degreeHistList.append(degreeHist)
			frequencyDegreeHistList.append(frequencyDegreeHist)
			diameterList.append(self.diameter())
			averageDegreeList.append(vertex_average(self.g, "total"))
			
		return degreeHistList, frequencyDegreeHistList, diameterList, averageDegreeList, GCCsizesList
		
	def dinamicGCCMetrics2(self, totalSteps = 26, intialTime = 126230461, timeInterval = 31536000):
		GCCsizes = []
		step = 0
		distanceList = []
		degreeHistList = []
		...
		while (step < totalSteps):
			...
			localClustList.append(localClust)
			localClustHistList.append(localClustHist)
			averageLocalClustList.append(vertex_average(self.g,localClust))

		return localClustList, localClustHistList, averageLocalClustList
		
									
	def dinamicGCCMetrics3(self, totalSteps = 26, intialTime = 126230461, timeInterval = 31536000):
		GCCsizes = []
		step = 0
		distanceList = []
		degreeHistList = []
	...
	def writeList(self, l, inputFileName="entrada", outputFileName="saida"):
		f = open(outputFileName + '_' + inputFileName, 'w')
		for x in l:
			f.writelines(str(x))
			f.write('\n')
		f.close()
	...
	def myMetrics(self, totalSteps = 26, intialTime = 126230461, timeInterval = 31536000):
		highestDegreeList = []
		ccSizes = []
		step = 0
		highestDegreesWithIndices = []
		
		currTime = intialTime
		highestDegreesWithIndices.append("[indices,degrees]")
		while (step < totalSteps):
			print "step = ", step
			step += 1
			currTime += timeInterval
			...
			print "intervalSet"
			self.setEdgeFilter("time")
			comp, CChist = label_components(self.g)
			ccSizes.append(CChist)
			print "filsetSet"
			u = self.extractGCC()
			print "num_vertices: ",u.num_vertices()
			
			#Pesquisar pelos 10 vertices com maior grau
			degreeList = u.degree_property_map("total").get_array()
			

			degreeList.sort()
			highestDegreeList.append(degreeList[-10:])

			indices = []
			degrees = []
			for v in u.vertices():
				if (v.out_degree() in degreeList[-10:]):
					indices.append(self.g.vertex_index[v])
					degrees.append(v.out_degree()) 
			highestDegreesWithIndices.append([indices,degrees])
			
			
			#histograma com numeros legiveis
			bin_counts, binRightEdges = vertex_hist(u, "total")
			
						
		return bin_counts, binRightEdges, highestDegreeList,highestDegreesWithIndices, ccSizes
